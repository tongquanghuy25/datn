...
const Statistic = () => {
    ...
    const mutation = useMutation({
        mutationFn: (data) => {
            ...
            return getStatisticBusOwner(JSON.parse(localStorage.getItem('bus_owner_id')), user?.access_token, data)
        },
        onSuccess: (data) => {
    ...
    console.log('day1', flexibleData);
    // console.log('day2', endDate);
    ...
    return (
        <div style={{ padding: '30px', background: '#ececec', height: "calc(100vh - 750px)" }}>
            <Row gutter={24}>
                <Col span={8}>
                    <StatisticCard
                        title='Số lượng xe'
                ...
                {activeTab === "1" ?
                    <>
                        ...
                        <DatePicker
                            ...
                            onChange={(time, timeString) => { setStartDate(time.format('YYYY-MM-DD')) }}
                        ...
                        <DatePicker
                            ...
                            onChange={(time, timeString) => { setEndDate(time.format('YYYY-MM-DD')) }}
                            disabledDate={(current) => current && current > dayjs().endOf('day')}
                        />
                    </>
                    : activeTab === "2" ?
